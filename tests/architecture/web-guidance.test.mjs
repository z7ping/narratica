...
test('小说创作流程只从真实作品状态推导当前进度', async () => {
  ...
  assert.match(source, /if \(mode === 'production'\)/)
  assert.match(source, /if \(mode === 'screenplay'\) return screenplayProgress/)
  assert.match(source, /不把导航位置伪装成真实生产进度/)
})

test('创作方法不伪造用户派生版本与作品绑定', async () => {
  const source = await read('packages/client/workspace/src/client/guidance.tsx')
  assert.match(source, /Narratica 官方方法 · 只读/)
  assert.match(source, /尚未接入真实保存能力/)
  assert.match(source, /不制造本地假版本/)
...
test('导演助手继续使用真实 DSH Session 且不展示固定伪调度列表', async () => {
  ...
  assert.match(styles, /\.director-body>\.small-card/)
